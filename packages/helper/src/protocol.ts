...
export const RequestResponseMap: Readonly<Record<string, RequestResponseEntry>> = {
  ...
  [SendMessageTypes.DAILY_REWARD_CLAIM]: actionResult(
    ReceiveMessageTypes.DAILY_REWARD_ACTION_RESULT,
    "claim"
  ),
};
...
export function dailyRewardClaimMessage(boostItemId: number | null = null): string {
  return buildMessage(SendMessageTypes.DAILY_REWARD_CLAIM, { boostItemId });
}
