...
describe("message payload schemas", () => {
  ...
  it("accepts daily reward claim send and action-result receive types from live traffic", () => {
    expect(
      validateMessagePayload("daily_reward_claim", "send", {
        boostItemId: null,
      }).status
    ).toBe("valid");
    expect(
      validateMessagePayload("daily_reward:action_result", "receive", {
        action: "claim",
        success: true,
        message: "Daily reward resgatada com sucesso.",
        claimedReward: {
          day: 2,
          type: "XP_BOOST",
          amount: {
